...
# If not exists the var, create it
if [ -z "$TEST_ENV" ]; then
  TEST_ENV=0
fi
...
if test $APP_LAMBDA_EVENT_SOURCE = true;then
  if test $TEST_ENV = 0; then
    ...
    echo '----------------------------------------'
    ${current_parent_folder}scripts/localstack/lambda/create-event-source-mapping-cloudwatch.sh $APP_LAMBDA_NAME $APP_RULE_NAME
  else
